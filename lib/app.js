...
app.put('/api/activities/:id/audit-history/:histId', wrap(async (req, res) => {
  const row = await getActivityRow(req.params.id);
  if (!row) return res.status(404).json({ error: 'Activité introuvable.' });
  const histId = Number(req.params.histId);
  const entry = (await db.execute({ sql: 'SELECT * FROM audit_history WHERE id = ? AND activityId = ?', args: [histId, req.params.id] })).rows[0];
  if (!entry) return res.status(404).json({ error: 'Audit introuvable.' });
  ...
  await db.execute({
    sql: `UPDATE audit_history SET total=?, scores=?, scoresNotes=?, scoresActions=?, auditOverview=?, auditAiInsight='' WHERE id=?`,
    args: [total, JSON.stringify(scores), JSON.stringify(notes), JSON.stringify(actions), overview, histId],
  });

  const latestId = (await db.execute({ sql: 'SELECT MAX(id) AS maxId FROM audit_history WHERE activityId = ?', args: [req.params.id] })).rows[0].maxId;
  if (Number(latestId) === histId) {
    await db.execute({
      sql: `UPDATE activities SET scores=?, scoresNotes=?, scoresActions=?, total=?, auditOverview=?, auditAiInsight='', auditDraft=0 WHERE id=?`,
      args: [JSON.stringify(scores), JSON.stringify(notes), JSON.stringify(actions), total, overview, req.params.id],
    });
  }
  res.json(await rowToActivity(await getActivityRow(req.params.id)));
}));
...
app.post('/api/backup/import', wrap(async (req, res) => {
  let s = req.body || {};
  const append = req.query.mode === 'append';
  if (append && typeof s.nom === 'string') s = { activities: [s] };
  if (!Array.isArray(s.activities)) return res.status(400).json({ error: 'Format de sauvegarde invalide.' });
  if (append) {
    if (!s.activities.length || s.activities.some(a => !a || typeof a.nom !== 'string' || !a.nom.trim() || (a.auditHistory != null && (!Array.isArray(a.auditHistory) || a.auditHistory.some(h => !h || typeof h !== 'object'))))) {
      return res.status(400).json({ error: 'Le fichier doit contenir au moins une activité nommée avec un historique valide.' });
    }
    for (const key of ['reviews', 'competitors']) {
      if (s[key] != null && (!Array.isArray(s[key]) || s[key].some(r => !r || typeof r !== 'object'))) {
        return res.status(400).json({ error: 'Format de sauvegarde invalide.' });
      }
    }
    const ids = new Map();
    for (const a of s.activities) {
      if (a.id && ids.has(a.id)) return res.status(400).json({ error: 'Identifiants d’activités dupliqués dans le fichier.' });
      if (a.id) ids.set(a.id, crypto.randomUUID());
    }
    s = {
      activities: s.activities.map(a => ({ ...a, id: ids.get(a.id) || crypto.randomUUID() })),
      reviews: (s.reviews || []).filter(r => ids.has(r.activityId)).map(r => ({ ...r, id: crypto.randomUUID(), activityId: ids.get(r.activityId) })),
      competitors: (s.competitors || []).filter(c => ids.has(c.activityId)).map(c => ({ ...c, id: crypto.randomUUID(), activityId: ids.get(c.activityId) })),
      dailyLogs: [],
    };
  }
